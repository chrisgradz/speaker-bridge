...
class SiriusXmAuthTests(unittest.TestCase):
    ...
    def test_account_full_includes_iheart_source_from_preserved_preset(self) -> None:
                ...
                store.set_preset(
                    "speaker-1",
                    {
                        ...
                        "raw_content_item": (
                            ...
                            'sourceAccount="listener@example.test" isPresetable="true">'
        ...
        self.assertIn("<username>listener@example.test</username>", xml)
    ...
    def test_configured_iheart_source_account_reads_env_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(path, "w", encoding="utf-8") as handle:
                handle.write("IHEART_SOURCE_ACCOUNT='listener@example.test'\n")

            account = configured_iheart_source_account(path=path, environ={})

        self.assertEqual(account, "listener@example.test")
    ...
    def test_build_play_content_item_renders_iheart_selection_as_direct_local_stream(self) -> None:
        ...
        self.assertIn('sourceAccount="listener@example.test"', raw)
    ...
    def test_push_station_to_speaker_reports_speaker_http_error_body(self) -> None:
        ...
        error = urllib.error.HTTPError(
            ...
            {},
            BytesIO(b"<error>INVALID_SOURCE</error>"),
        )
    ...
    def test_play_page_exposes_station_browser(self) -> None:
        ...
        self.assertIn("data-source=\"TUNEIN\"", PLAY_HTML)
        self.assertIn("data-source=\"IHEART\"", PLAY_HTML)
        self.assertNotIn("data-source=\"LOCAL_INTERNET_RADIO\"", PLAY_HTML)
        self.assertNotIn("Direct Stream", PLAY_HTML)
        self.assertNotIn("pushDirectStream", PLAY_HTML)
    ...
    def test_play_routes_are_registered(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                paths = [
                    "/play",
                    "/api/speakers/TESTDEVICE01/play",
                    "/api/experiments/play/speakers/TESTDEVICE01/select",
                ]
                matched = {
                    path: any(
        ...
        self.assertEqual(
            ...
            {
                "/play": True,
                "/api/speakers/TESTDEVICE01/play": True,
                "/api/experiments/play/speakers/TESTDEVICE01/select": True,
            },
        )
